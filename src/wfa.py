...
class WFA:
  ...
  def add_path(self, tokens, weight):
    ...
    cum_weight = self.sr.one
    for token in tokens:
      state, w = self.next_state(last_state, token)
      if state is None:
        state = self.new_state()
        self.add_edge(last_state, token, state, None)
      elif w is not None:
        cum_weight = self.sr.mul(cum_weight, w)
      last_state = state
    weight_left = self.sr.left_divide(weight, cum_weight)
    self.weights[state] = weight_left

  def grow_trie(self, strings, weights=None) -> None:
    """Traverse the WFA with strings and add trie paths for missing transitions.

    If no weights are provided, each string is labelled with weight one.
    ...
    for idx, string in enumerate(strings):
      if weights is not None:
        self.add_path(string, weights[idx])
      else:
        self.add_path(string, self.sr.one)
